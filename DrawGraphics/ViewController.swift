...
class ViewController: UIViewController {
    ...
    @IBAction func btnDrawLine(_ sender: UIButton) {
        UIGraphicsBeginImageContext(imageView.frame.size) // context를 이미지 뷰와 같은 크기로 설정
        let context = UIGraphicsGetCurrentContext()! // 생성한 컨텍스트 정보를 가져옴
        
        //Draw Line
        context.setLineWidth(2.0) // 선을 굵기 설정
        context.setStrokeColor(UIColor.red.cgColor) // 선의 색상 설정
        
        context.move(to: CGPoint(x: 70, y:50)) // 커서의 시작 위치 설정
        context.addLine(to: CGPoint(x: 270, y: 250)) // 지정한 위치까지 선을 이어줌
